...
(define (new-table-producer)
  (define str (bytes->string/latin-1 (file->bytes "list_ascii.txt")))
  (define (helper n)
    (if (= n 256) `()
      (append (list (list (substring str n (+ n 1)) (extend-bn (number->binary n) 9))) (helper (+ n 1)))))
  (helper 0))

(define (table-producer l n)
  (if (= n 129) (append l (list (list (substring 
...
(define basic-table (new-table-producer))

(define (compress-basic input-path output-path method)
  (begin
    (set! basic-table (new-table-producer))
  (display-to-file (list->bytes (string-to-byte (method (string-append (file->string input-path) "##")))) output-path #:exists 'replace)))

(define (compress-advanced input-path output-path method)
  (begin
    (set! basic-table (table-producer `() 1))
  (display-to-file (list->bytes (string-to-byte (method (string-append (bytes->string/latin-1 (file->bytes input-path)) "##")))) output-path #:exists 'replace)))
